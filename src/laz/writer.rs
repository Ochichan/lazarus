...
impl LazPackage {
    ...
    pub fn export<P: AsRef<Path>>(&mut self, path: P) -> Result<()> {
        ...
        let options_store = FileOptions::default()
            .compression_method(CompressionMethod::Stored);
        
        zip.start_file("mimetype", options_store).map_err(zip_err)?;
        zip.write_all(MIMETYPE.as_bytes()).map_err(|e| LazarusError::Io(e))?;
        ...
        let curriculum_json = serde_json::to_string_pretty(&self.curriculum)
            .map_err(|e| LazarusError::Serialize(e.to_string()))?;
        write_json(&mut zip, "curriculum.json", &curriculum_json, &mut manifest_files)?;
        ...
        zip.start_file("manifest.json", options_deflate).map_err(zip_err)?;
        zip.write_all(manifest_json.as_bytes()).map_err(|e| LazarusError::Io(e))?;
    ...
    pub fn add_asset_to_zip<P: AsRef<Path>>(
        ...
        let filename = path.file_name()
            .and_then(|n| n.to_str())
            .ok_or_else(|| LazarusError::Io(std::io::Error::new(
...
fn write_json(
    ...
    zip.write_all(content.as_bytes()).map_err(|e| LazarusError::Io(e))?;
...
fn zip_err(e: zip::result::ZipError) -> LazarusError {
    LazarusError::Io(std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))
}
